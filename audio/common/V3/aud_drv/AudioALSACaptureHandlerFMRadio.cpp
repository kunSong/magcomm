...
status_t AudioALSACaptureHandlerFMRadio::open()
{
    ...
    if ((AudioALSAStreamManager::getInstance()->isModeInVoipCall() == true) || (mSupportConcurrencyInCall == false && (AudioALSAStreamManager::getInstance()->isModeInPhoneCall() == true)))
    {
        mCaptureDataClient = NULL;
        ALOGD("-%s() don't support FM Record at incall\VoipCall mode", __FUNCTION__);
        return NO_ERROR;
    }
