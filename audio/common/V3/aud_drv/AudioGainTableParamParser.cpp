...
const GAIN_MIC_MODE gppSpeechMicMap[NUM_GAIN_SPEECH_BAND][NUM_GAIN_DEVICE] = {
    {Normal_Mic, Headset_Mic, Handfree_Mic, GAIN_MIC_INVALID, GAIN_MIC_INVALID, GAIN_MIC_INVALID, GAIN_MIC_INVALID},
    {Normal_WB_Mic, Headset_WB_Mic, Handfree_WB_Mic, GAIN_MIC_INVALID, GAIN_MIC_INVALID, GAIN_MIC_INVALID, GAIN_MIC_INVALID}
};

// VoIP
const GAIN_MIC_MODE gppVoipMicMap[NUM_GAIN_DEVICE] = {
    VOIP_Normal_Mic, VOIP_Headset_Mic, VOIP_Handfree_Mic, GAIN_MIC_INVALID, GAIN_MIC_INVALID, GAIN_MIC_INVALID, GAIN_MIC_INVALID
};
    ...
    for (int stream = GAIN_MIN_STREAM_TYPE; stream <= GAIN_MAX_STREAM_TYPE; stream++)
    {
        for (int device = 0; device < NUM_GAIN_DEVICE; device++)
        {
            ...
            for (int i = 0; i < arraySize; i++)
            {
                ...
                {
                    ALOGW("error, param out of range, val %d > %d"

, shortArray[i], mSpec.digiDbMax);
                    *digital = 0;
                }
...
status_t GainTableParamParser::updatePlaybackAnalogGain(GainTableParam *_gainTable)
{
    ...
    for (int stream = GAIN_MIN_STREAM_TYPE; stream <= GAIN_MAX_STREAM_TYPE; stream++)
    {
        for (int device = 0; device < NUM_GAIN_DEVICE; device++)
        {
            ...
            if (param_hs->arraySize != 1 || param_spk->arraySize != 1)
            {
                ALOGW("warn: %s arraySize(%d) != 1 || %s arraySize(%d) != 1", paramHsName,
                                                                              param_hs->arraySize,
                                                                              paramSpkName,
                                                                              param_spk->arraySize);
            }
...
status_t GainTableParamParser::updateSpeechVol(GainTableParam *_gainTable)
{
    ...
    for (int band = GAIN_SPEECH_NB; band <= GAIN_SPEECH_WB; band++)
    {
        for (int device = 0; device < NUM_GAIN_DEVICE; device++)
        {
            ...
            // Uplink gain
            const GAIN_MIC_MODE micType = gppSpeechMicMap[band][device];
            if (micType >= 0 && micType < NUM_GAIN_MIC_MODE)
            {
                ...
                _gainTable->micGain[micType].gain = *(int*)param_ul_gain->data;
            }
...
status_t GainTableParamParser::updateRecordVol(GainTableParam *_gainTable)
{
    ...
    const int numRecordMicType = 17;
    std::string paramPath[numRecordMicType] = {"Application,Sound recording,Profile,Handset",
                                               "Application,Sound recording,Profile,Headset",
                                               "Application,Camera recording,Profile,Handset",
                                               "Application,Camera recording,Profile,Headset",
                                               "Application,ASR,Profile,Handset",
                                               "Application,ASR,Profile,Headset",
                                               "Application,VR,Profile,Handset",
                                               "Application,VR,Profile,Headset",
                                               "Application,VOICE_UNLOCK,Profile,Handset",
                                               "Application,VOICE_UNLOCK,Profile,Headset",
                                               "Application,CUSTOMIZATION2,Profile,Handset",
                                               "Application,CUSTOMIZATION2,Profile,Headset",
                                               "Application,CUSTOMIZATION3,Profile,Handset",
                                               "Application,CUSTOMIZATION3,Profile,Headset",
                                               "Application,TTY_CTM_Mic",
                                               "Application,Level_Shift_Buffer_Gain",
                                               "Application,Analog_PLay_Gain"
                                               };

    int targetMicType[numRecordMicType] = {Idle_Normal_Record,
                                           Idle_Headset_Record,
    ...
    for (int i = 0; i < numRecordMicType; i++)
    {
        // get param unit using param path
        ALOGD("paramPath = %s", paramPath[i].c_str());

        paramUnit = audioTypeGetParamUnit(audioType, paramPath[i].c_str());
        if (!paramUnit)
        {
            ALOGW("error: get paramUnit fail, paramPath = %s", paramPath[i].c_str());
            continue;
        }
        ...
        int micType = targetMicType[i];
        if (*(int*)param_ul_gain->data > mSpec.micIdxMax ||
            *(int*)param_ul_gain->data < mSpec.micIdxMin )
        {
            ALOGW("error, ul_gain = %d out of bound, paramPath = %s", *(int*)param_ul_gain->data, paramPath[i].c_str());
        }
        _gainTable->micGain[micType].gain = *(int*)param_ul_gain->data;
        ALOGD(

"\tgain = %d", _gainTable->micGain[micType].gain);
    }
...
status_t GainTableParamParser::updateVoIPVol(GainTableParam *_gainTable)
{
    ...
    for (int device = 0; device < NUM_GAIN_DEVICE; device++)
    {
        ...
        // Uplink gain
        const GAIN_MIC_MODE micType = gppVoipMicMap[device];
...
template<class T>
status_t GainTableParamParser::getParam(ParamUnit *_paramUnit, T *_param, char *_paramName)
{
    Param *param;
...
status_t GainTableParamParser::getParam(ParamUnit *_paramUnit, std::string *_param, char *_paramName)
{
    Param *param;
    param = paramUnitGetParamByName(_paramUnit, _paramName);
    if (!param)
...
template<class T>
status_t GainTableParamParser::getParamVector(ParamUnit *_paramUnit, std::vector<T> *_param, char *_paramName)
{
    Param *param;
    param = paramUnitGetParamByName(_paramUnit, _paramName);
    if (!param)
...
status_t GainTableParamParser::getParamVector(ParamUnit *_paramUnit, std::vector<std::string> *_param, char *_paramName)
{
    Param *param;
    param = paramUnitGetParamByName(_paramUnit, _paramName);
    if (!param)
