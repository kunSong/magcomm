...
status_t SpeechMessengerCCCI::Initial()
{
    ...
    mSphCCCITmpBuf = new char[mM2AShareBufLen];
...
status_t SpeechMessengerCCCI::WaitUntilModemReady()
{
    ...
    do
    {
        ...
        {
            ALOGW("Wait CCCI open #%d times, modem current status = %c, errno: %d", ++trycnt, status, errno);
            sph_msleep(100);
...
int32_t SpeechMessengerCCCI::SendMessage(const ccci_buff_t &ccci_buff)
{
    ...
    {
        ALOGE("%s(), modem_status(%c) != MODEM_STATUS_READY, errno: %d, mIsModemEPOF = %d", __FUNCTION__, modem_status, errno, mIsModemEPOF);
    ...
    while (pCCCI->CCCIEnable)
    {
        ...
        switch (m2a_message_id)
        {
            ...
            case MSG_M2A_NETWORK_STATUS_NOTIFY:
            {
                ...
                bool nb = (ccci_buff.message & 1) ? false : true;
...
//    char SeqPcmBuf[CCCI_MAX_PAYLOAD_SIZE * 4 + 16] = {0};
    ...
    {
        ALOGV("%s(), message = 0x%x, mSphCCCITmpBuf(0x%x), ul_ring_buf.pRead(0x%x), BytesCopied(0x%x)",
    ...
    switch (mPcmRecordType)
    {
        ...
        case RECORD_TYPE_DL:
            ...
            //downlink raw pcm
            PtrTarget = SeqPcmBuf + BytesCopied;
            PtrSource = (char *)mSphCCCITmpBuf + CCCI_SHARE_BUFF_HEADER_LEN + CCCI_RAW_PCM_BUFF_HEADER_LEN + header_RawPcmBufInfo.u16ULLength;
    ...
    ALOGD("-%s(), pBufBase(0x%x), bufLen(0x%x), BytesCopied(0x%x), pRead(0x%x), pWrite(0x%x)",
          __FUNCTION__, ul_ring_buf.pBufBase, ul_ring_buf.bufLen, BytesCopied, ul_ring_buf.pRead, ul_ring_buf.pWrite);
...
status_t SpeechMessengerCCCI::SetPcmRecordType(record_type_t type_record)
{
