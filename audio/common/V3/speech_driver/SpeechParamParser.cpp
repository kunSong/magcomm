...
status_t SpeechParamParser::GetSpeechParamFromAppParser(uint16_t uSpeechTypeIndex, AUDIO_TYPE_SPEECH_LAYERINFO_STRUCT *mParamLayerInfo, char *pPackedParamUnit, uint16_t *sizeByteTotal)
{
    ...
    if (&mAppHandle == NULL)
    {
        ALOGE("%s() &mAppHandle == NULL", __FUNCTION__);
...
#else
    ALOGD("%s(), categoryType.size=%d, paramName.size=%d", __FUNCTION__, mParamLayerInfo->categoryType.size(), mParamLayerInfo->paramName.size());
    for (u4Index = 0; u4Index < mParamLayerInfo->categoryType.size() ; u4Index++)
    {
        ALOGD("%s(), categoryType[%d]= %s", __FUNCTION__, u4Index, mParamLayerInfo->categoryType.at(u4Index).string());
    }
    for (u4Index = 0; u4Index < mParamLayerInfo->categoryName.size() ; u4Index++)
    {
        ALOGD("%s(), categoryName[%d]= %s", __FUNCTION__, u4Index, mParamLayerInfo->categoryName.at(u4Index).string());
    }
    ...
    ALOGD("%s() audioTypeName=%s", __FUNCTION__, mParamLayerInfo->audioTypeName);
    /* Query AudioType */
    ...
    ALOGD("%s() paramId=0x%x", __FUNCTION__, paramUnit->paramId);
...
int SpeechParamParser::GetDmnrParamUnit(char *pPackedParamUnit)
{
    ...
    memset(pPackedParamUnitFromApp, 0, sizeof(pPackedParamUnitFromApp));
    ...
    {
        ALOGD("%s(), paramName[%d]= %s", __FUNCTION__, u4Index, pParamLayerInfo.paramName.at(u4Index).string());
    }
...
//int SpeechParamParser::GetSpeechParamUnit(char *pPackedParamUnit, speech_mode_t IdxMode, char IdxVolume)
{
    ...
    {
        ALOGD("%s(), categoryType[%d]= %s", __FUNCTION__, u4Index, pParamLayerInfo.categoryType.at(u4Index).string());
    }
    for (u4Index = 0; u4Index < pParamLayerInfo.paramName.size() ; u4Index++)
    {
        ALOGD("%s(), paramName[%d]= %s", __FUNCTION__, u4Index, pParamLayerInfo.paramName.at(u4Index).string());
    ...
    {
        ALOGD("%s(), categoryName[%d]= %s", __FUNCTION__, u4Index, pParamLayerInfo.categoryName.at(u4Index).string());
    }
