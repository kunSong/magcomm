...
#ifdef DEN_PHONE_MIC_INVERSE
    mMicInverse(true),	// if defined DEN_PHONE_MIC_INVERSE, default invert main & ref mic
#else
	mMicInverse(false),
#endif
...
{
    ALOGD("+%s(), new_device = 0x%x, mInputDevice = 0x%x, mStartInputDeviceCount = %d, mMicInverse=%d", __FUNCTION__, new_device, mInputDevice, mStartInputDeviceCount,mMicInverse);
    ...
    {
        ALOGW("%s(), no need to config analog part", __FUNCTION__);
        return NO_ERROR;
    }
...
{
    ALOGD("+%s(), mInputDevice = 0x%x, stop_device = 0x%x, mStartInputDeviceCount = %d, mMicInverse=%d", __FUNCTION__, mInputDevice, stop_device, mStartInputDeviceCount, mMicInverse);
