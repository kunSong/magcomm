...
ssize_t AudioALSAStreamIn::read(void *buffer, ssize_t bytes)
{
    ...
    int tryCount = 10;
    while(mSuspendLockCount && tryCount-- ) {
        ALOGD("%s, free CPU, mSuspendLockCount = %d, tryCount %d", __FUNCTION__, mSuspendLockCount, tryCount);
        usleep(300);
    }

    mSuspendLock.lock_timeout(3000);
...
status_t AudioALSAStreamIn::setParameters(const String8 &keyValuePairs)
{
    ...
    if (param.getInt(keyRouting, value) == NO_ERROR)
    {
        ...
        if (status != NO_ERROR)
        {
            mStreamAttributeTarget.input_device = inputdevice;
        }
        android_atomic_dec(&mLockCount);
    }
...
status_t AudioALSAStreamIn::routing(audio_devices_t input_device)
{
    ...
    if (mStandby == true)
    {
        ...
        ALOGD("%s(),mStreamAttributeTarget.input_device = 0x%x", __FUNCTION__,mStreamAttributeTarget.input_device);
    }
    else
    {
...
status_t AudioALSAStreamIn::setSuspend(const bool suspend_on)
{
    ...
    android_atomic_inc(&mSuspendLockCount);
    AudioAutoTimeoutLock suspendLock(mSuspendLock);
    android_atomic_dec(&mSuspendLockCount);
    ...
    else if (suspend_on == false)
    {
        ...
        {
            ALOGW("%s(), mSuspendCount <= 0 no need to resume, suspend_on = %d", __FUNCTION__, suspend_on);
        }
    }
