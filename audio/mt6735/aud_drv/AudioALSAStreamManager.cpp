    ...
    mBypassDualMICProcessUL(false)
{
    ...
    appHandleRegXmlChangedCb(mAppHandle, CallbackAudioXmlChanged);
#endif
    ...
    uint32_t input_flag)
{
    ...
    {
        input_device = CheckInputDevicePriority(input_device,false);
    ...
    stream_attribute_t *stream_attribute_source)
{
    ...
    else
    {
        switch (stream_attribute_source->output_devices)
        {
            ...
            {
#if !defined(MTK_BASIC_PACKAGE)
                if(stream_attribute_source->audio_format == AUDIO_FORMAT_MP3)
    ...
    stream_attribute_t *stream_attribute_target)
{
    ...
    const audio_devices_t current_output_devices = (mStreamOutVector.size() > 0)
                                                   ? mStreamOutVector[0]->getOutputDevice()
                                                   : AUDIO_DEVICE_NONE;
...
int AudioALSAStreamManager::SetCaptureGain(void)
{
    ...
    {
        //if(mStreamInVector[i]->getOutputDevice() == output_devices)
        {
...
status_t AudioALSAStreamManager::setMode(audio_mode_t new_mode)
{
    ...
    if (mFMController->getFmEnable() && isModeInPhoneCall(new_mode))
    {
        setFmEnable(false);
    }
...
status_t AudioALSAStreamManager::routingOutputDevice(AudioALSAStreamOut *pAudioALSAStreamOut, const audio_devices_t current_output_devices, audio_devices_t output_devices)
{
    ...
    if (isModeInPhoneCall() == true)
    {
        ...
        //if (pAudioALSAStreamOut->getOutputDevice() == current_output_devices) // TODO(Harvey): or add group?
        {
            status = pAudioALSAStreamOut->routing(output_devices);
...
status_t AudioALSAStreamManager::routingInputDevice(AudioALSAStreamIn *pAudioALSAStreamIn, const audio_devices_t current_input_device, audio_devices_t input_device)
{
    ...
    if (sharedDevice && (mStreamInVector.size() > 1))
    {
        input_device = CheckInputDevicePriority(input_device,true);
    }
...
audio_devices_t AudioALSAStreamManager::CheckInputDevicePriority(audio_devices_t input_device, bool routing)
{
   for (size_t i = 0; i < mStreamInVector.size(); i++)
   {
        //if ((setUsedDevice(input_device) == 0) || ((routing == true) && (mStreamInVector[i]->getStandby() != true)))
   ...
   ALOGD("%s(), routing = %d, input_device = 0x%x", __FUNCTION__, routing, input_device);
   return input_device;
}
...
status_t AudioALSAStreamManager::standbyAllInputStreams(const bool setModeRequest)
{
    ...
    for (size_t i = 0; i < mStreamInVector.size(); i++)
    {
        ...
        {
            ALOGD("%s(), mStreamInVector[%d] capture handler not created yet, pAudioALSAStreamIn=%p", __FUNCTION__, i,pAudioALSAStreamIn);
            continue;
        }
...
status_t AudioALSAStreamManager::setMDVolumeIndex(int stream, int device, int index)
{
    ...
    {
        ALOGD("-%s() not in incall mode", __FUNCTION__);
    }
    return NO_ERROR;
}
